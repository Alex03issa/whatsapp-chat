...
//code fro get contact info chates names and avatars img

/**import React, { useState, useEffect, useRef } from "react";
import { getChats, getContactInfo } from "../services/api";
import { useNavigate } from "react-router-dom";
import ChatWindow from "../components/ChatWindow";
import { FaEllipsisV, FaEdit } from "react-icons/fa";
import "./Home.css";
...
const Home = ({ setCredentials, credentials }) => {
    const [chats, setChats] = useState([]);
    const [chatDetails, setChatDetails] = useState({}); // Store chat names & avatars
    const [searchTerm, setSearchTerm] = useState("");
    const [selectedChat, setSelectedChat] = useState(null);
    const [newChatNumber, setNewChatNumber] = useState("");
    const [showModal, setShowModal] = useState(false);
    ...
    const fetchChats = async (idInstance, apiTokenInstance) => {
        ...
        try {
            ...
            // Fetch contact info for each chat
            const chatDetailsTemp = {};
            for (const chat of chatList) {
                try {
                    const contactInfo = await getContactInfo(idInstance, apiTokenInstance, chat.id);
                    chatDetailsTemp[chat.id] = {
                        name: contactInfo.name || contactInfo.contactName || chat.id,
                        avatar: contactInfo.avatar || `https://ui-avatars.com/api/?name=${chat.id}&background=random`,
                    };
                } catch (error) {
                    console.warn(`⚠️ Could not fetch contact info for ${chat.id}`);
                }
            }
            setChatDetails(chatDetailsTemp);
        } catch (error) {
            console.error("❌ Error fetching chats:", error);
            setChats([]);
        }
        ...
        <div className="home-container">
            ...
            <div className="sidebar">
                ...
                <ul className="chat-list">
                    {chats.length > 0 ? (
                        chats
                            .filter(chat => (chatDetails[chat.id]?.name || chat.id).toLowerCase().includes(searchTerm.toLowerCase()))
                            .map((chat, index) => (
                                <li 
                                    key={index} 
                                    className={selectedChat?.id === chat.id ? "selected-chat" : ""}
                                ...
                                >
                                    <div className="chat-avatar">
                                        <img src={chatDetails[chat.id]?.avatar} alt="Avatar" />
                                    </div>
                                    <div className="chat-info">
                                        <span className="chat-name">{chatDetails[chat.id]?.name}</span>
                                    </div>
                                </li>
                            ))
                    ) : (
